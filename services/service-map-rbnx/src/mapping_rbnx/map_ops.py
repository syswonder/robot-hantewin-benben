...
def _publish_full_map(node, timeout_s: float = 30.0, optimized: bool = True) -> tuple[bool, str]:
    """Ask RTAB-Map to republish the global map.

    ``optimized=True`` (default; the "after load" use) runs a full graph
    optimization before republishing. During save that re-optimization
    re-anchors map→odom and teleports the robot (observed ~1.7 m jump,
    which then gets baked in as an extra region), so save passes
    ``optimized=False`` to republish from the live graph without moving poses.
    """
    try:
    ...
    return True, f"published {'optimized' if optimized else 'live'} global map"
...
def load_map_impl(map_id: str, mode: str = "localization",
    ...
    try:
        ...
        _probe_pose(map_id, "load:before_switch")
        ok2, info2 = _set_mode(node, mode)
        _probe_pose(map_id, "load:after_switch")
        if not ok2:
        ...
        _probe_pose(map_id, "load:before_loaddb")
        ok, load_detail = _load_database(node, runtime_db, load_timeout_s)
        _probe_pose(map_id, "load:after_loaddb")
        if not ok:
            return {"ok": False,
        ...
        _probe_pose(map_id, "load:before_publish")
        pub_ok, pub_detail = _publish_full_map(node, timeout_s=publish_timeout_s)
        _probe_pose(map_id, "load:after_publish")
        if not pub_ok:
            node.destroy_subscription(barrier["subscription"])
            return {"ok": False, "detail": f"loaded {map_id}, but full map publish failed: {pub_detail}"}
        # Seed the initial pose BEFORE verifying: localization only converges —
        # and only then republishes the full map — once a pose anchors the robot.
        seeded = ""
        if has_initial_pose:
        ...
        if not has_initial_pose:
            # No map-frame anchor yet, so the full map cannot appear in the
            # verify window; a short probe is enough to confirm the DB is loaded,
            # then report success and let the caller seed a pose estimate.
            verify_timeout_s = min(verify_timeout_s, 5.0)
        verified, verify_detail = _finish_target_map_wait(node, barrier, verify_timeout_s)
        if not verified and not has_initial_pose:
            log.warning("load_map[%s] full-map verify deferred (no initial pose): %s",
                        map_id, verify_detail)
            verify_detail = "map loaded; give a pose estimate to display the full map"
        elif not verified:
...
def _probe_pose(ctx: str, tag: str) -> dict:
    """Diagnostic: log the robot's map-frame pose so a save/load/switch-triggered
    teleport can be attributed to a specific step. Returns the get_pose_impl dict
    (ok=False → pose topic unreadable / not localized at that instant)."""
    p = get_pose_impl(timeout_s=1.0)
    if p["ok"]:
        log.info("pose_probe[%s] %s x=%.3f y=%.3f yaw=%.3f (%s)",
                 ctx, tag, p["x"], p["y"], p["theta"], p["frame_id"])
    else:
        log.info("pose_probe[%s] %s unavailable: %s", ctx, tag, p["detail"])
    return p
...
def switch_mode_impl(mode: str) -> dict:
    ...
    if node is None:
        return {"ok": False, "detail": "rclpy node unavailable (ROS not running?)"}
    try:
        p0 = get_pose_impl(timeout_s=1.0)
        ok, info = _set_mode(node, mode)
        p1 = get_pose_impl(timeout_s=1.0)
        if p0["ok"] and p1["ok"]:
            log.info("switch_mode[%s] pose before=(%.3f,%.3f,%.3f) after=(%.3f,%.3f,%.3f) delta=%.3fm",
                     mode, p0["x"], p0["y"], p0["theta"], p1["x"], p1["y"], p1["theta"],
                     math.hypot(p1["x"] - p0["x"], p1["y"] - p0["y"]))
        else:
            log.info("switch_mode[%s] pose before_ok=%s after_ok=%s (before=%s / after=%s)",
                     mode, p0["ok"], p1["ok"], p0.get("detail", ""), p1.get("detail", ""))
        if not ok:
...
def _flush_rtabmap_database(node, live_db: str, timeout_s: float = 180.0) -> tuple[bool, str, str]:
    """Pause RTAB-Map and snapshot the live DB file WITHOUT re-anchoring.

    The previous implementation used RTAB-Map's /backup service, which
    serializes memory and then *reinitializes* the database — reloading the
    graph and re-anchoring map→odom. With a drifting wheel-odometry correction
    that teleports the robot by the accumulated drift on every save (probes
    measured ~3-8 m jumps on a *stationary* robot, landing right after the
    /backup call). So save must not call /backup: pause processing (stops the
    incremental DB writer), take a transactional SQLite snapshot, then resume.
    map→odom is never touched, so the robot pose is unchanged across save.

    RTAB-Map writes node data incrementally during mapping, so the paused live
    DB is current up to the pause instant; the transactional snapshot avoids
    any mid-write copy. If pause/resume services are unavailable, saving fails
    safely instead of racing the writer.
    """
    pause_ok, pause_detail = _set_rtabmap_paused(node, True, timeout_s=10.0)
    if not pause_ok:
        return False, pause_detail, ""
    snap = ""
    try:
        if not os.path.isfile(live_db):
            return False, f"live database not found: {live_db}", ""
        snap = f"{live_db}.snap-{os.getpid()}"
        if os.path.exists(snap):
            os.remove(snap)
        ok, detail = _sqlite_backup(live_db, snap)
        if not ok:
            return False, f"live database snapshot failed: {detail}", ""
        ok2, detail2 = _sqlite_quick_check(snap)
        if not ok2:
            try:
                os.remove(snap)
            except OSError:
                pass
            return False, f"live database snapshot integrity failed: {detail2}", ""
        return True, "rtabmap paused; live DB snapshot taken; resumed", snap
    finally:
        resume_ok, resume_detail = _set_rtabmap_paused(node, False, timeout_s=10.0)
        if not resume_ok:
            log.warning("rtabmap resume failed after snapshot: %s", resume_detail)
...
def save_map_impl(map_id: str, note: str = "",
    ...
    try:
        ...
        def _probe(tag: str) -> None:
            """Log the robot's map-frame pose so a save-triggered teleport can
            be attributed to a specific step (flush / sqlite / publish / preview)."""
            p = get_pose_impl(timeout_s=1.0)
            if p["ok"]:
                log.info("save_map[%s] pose_probe %s x=%.3f y=%.3f yaw=%.3f (%s)",
                         map_id, tag, p["x"], p["y"], p["theta"], p["frame_id"])
            else:
                log.info("save_map[%s] pose_probe %s unavailable: %s",
                         map_id, tag, p["detail"])
        ...
        pub_ok, pub_detail = _publish_full_map(
            node,
            timeout_s=float(os.environ.get("MAPPING_PUBLISH_MAP_TIMEOUT_S", "45")),
            optimized=False,  # no full re-optimization on save: it re-anchors map→odom and teleports the robot
        )
        if not pub_ok:
            return {
                "ok": False,
