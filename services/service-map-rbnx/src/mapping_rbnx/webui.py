...
*see* the live SLAM map and pose, and drive the runtime map operations:
  ...
  - Pose Estimate → press on the map to place, drag to aim heading, release
    to seed pose_estimate(x, y, theta) (RViz 2D-Pose-Estimate-style)
...
<div class=wrap>
 <div class=card>
  ...
  <div class=muted>wheel = zoom ·
   <button id=btn-tool-pan class="alt active" style="padding:2px 8px" onclick="setTool('pan')">Pan (平移)</button>
   <button id=btn-tool-pose style="padding:2px 8px" onclick="setTool('pose')">Pose (定位·拖拽定方向)</button>
   · <button class=alt style="padding:2px 8px" onclick="fitView()">Fit</button>
   <span id=toolhint class=muted style="margin-left:6px"></span></div>
 </div>
...
// ── interactive canvas map: pan / zoom / grid / pose tool (press+aim+release)
...
function draw(){fit();cx.clearRect(0,0,cv.width,cv.height);
 ...
 // pose-tool preview: green marker + heading arrow while press-dragging
 if(posePlacing){let p=w2p(posePlacing.wx,posePlacing.wy),th=posePlacing.theta;
  cx.fillStyle='#2bd66f';cx.strokeStyle='#2bd66f';cx.lineWidth=2;
  cx.beginPath();cx.arc(p[0],p[1],6,0,7);cx.fill();
  cx.beginPath();cx.moveTo(p[0],p[1]);cx.lineTo(p[0]+24*Math.cos(th),p[1]-24*Math.sin(th));cx.stroke()}}
...
// Tool: 'pan' (drag to pan) or 'pose' (RViz-style press→place, drag→aim, release→seed).
let TOOL='pan', posePlacing=null;
function setTool(t){TOOL=t;
 document.getElementById('btn-tool-pan').classList.toggle('active',t==='pan');
 document.getElementById('btn-tool-pose').classList.toggle('active',t==='pose');
 document.getElementById('toolhint').textContent =
  t==='pose'?'按下定位 → 拖拽定方向 → 松开发送':'拖拽平移 · 滚轮缩放 · 双击适配'}
setTool('pan');
cv.addEventListener('mousedown',e=>{
 if(TOOL==='pose'&&MI){let p=pt(e),wp=p2w(p[0],p[1]);
  posePlacing={px:p[0],py:p[1],wx:wp[0],wy:wp[1],theta:0};
  moved=0;drag=null;userMoved=true;draw();return}
 drag=pt(e);moved=0});
window.addEventListener('mousemove',e=>{
 if(posePlacing){let p=pt(e);                       // pose tool: drag sets heading
  posePlacing.theta=Math.atan2(-(p[1]-posePlacing.py),p[0]-posePlacing.px);
  moved+=Math.abs(p[0]-posePlacing.px)+Math.abs(p[1]-posePlacing.py);draw();return}
 if(!drag)return;let p=pt(e);                        // pan tool
...
window.addEventListener('mouseup',async e=>{
 if(posePlacing){let {wx,wy,theta}=posePlacing;posePlacing=null;  // release = seed
  if(MI){setStatus('seeding pose…');
   let r=await (await fetch('/api/pose_estimate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({x:wx,y:wy,theta})})).json();
   setStatus(r.detail||('seeded ('+wx.toFixed(2)+', '+wy.toFixed(2)+', '+(theta*180/Math.PI).toFixed(0)+'°)'))}
  draw();return}
 drag=null});
